...
# Pastikan semua path relative kepada lokasi app.py — fix untuk Streamlit Cloud
...
def load_template(filename: str) -> str:
    """Load HTML template dari folder templates/ — path relative ke app.py."""
...
if "🆕 Jana Kad Baru" in page:
    ...
    if st.button("✨ Jana Kad Sekarang!", disabled=bool(missing)):
        ...
        if template_html is None:
            st.error(f"❌ Template fail tidak dijumpai: `templates/{selected_tmpl['file']}`\n\nPastikan fail HTML template ada dalam folder `templates/` bersebelahan dengan `app.py`")
        else:
